<template>
    <div class='page'>
        <div class='search-top'>
            <div class='search-header'>
                ...
                <div class='screen-btn'>
					<span
                            @click="trysouch()"
                            style="font-size: 0.235rem;margin-left: 0.2rem;"
                            v-show="$route.query.livetitle">选择</span>
                </div>
        ...
        <div class='goods-main'>
            <div
                    :class="{'goods-list':true,active123456:item.istochoose}"
                    :key="index" @click="totototo(item)"
        ...
        <my-search :isLocal="true" :show="searchShow"></my-search>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            ...mapMutations({
                ...
                SET_LIVE_SHOPTWO: "live/SET_LIVE_SHOPTWO"
            }),
            trysouch() {
                // searchData
                let itemlist = []
                for (let i = 0; i < this.searchData.length; i++) {
                    if (this.searchData[i].istochoose) {
                        itemlist.push(this.searchData[i])
                    }
                }
                this.SET_LIVE_SHOPTWO({itemlist})
                this.$router.push('/live?active=3')
            },
            totototo(item) {

                if (this.$route.query.livetitle) {
                    item.istochoose = !item.istochoose
                } else {
                    this.$router.push('/goods/details?productID=' + item.productID)
                }
